...
export default function AdventureDetail({ adventure, guide, lang }: AdventureContentProps) {
  ...
  return (
    <div className="container px-[5%] 2xl:px-[15%] mx-auto text-primary">
      ...
      <div className="divider w-full my-4 border-primary/20 border-1"></div>

      {/* Content */}
      <section className="flex pb-12 text-primary">
        <div className="grid gap-4 lg:grid-cols-3 w-full">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="rounded-xl bg-white/50 p-4 text-left flex flex-col shadow-sm">
              <h2 className="mb-4 text-xl font-bold text-primary">{t.about_experience || "Sobre a Experiência"}</h2>
              <p className="text-muted-foreground leading-relaxed min-h-[200px]">{adventure.description}</p>
            </div>
          ...
          <div className="" >
            {/* CTA Card Desktop */}
            <div className="rounded-xl mb-4 shadow px-6 py-4 bg-white hidden md:block">
              <Link
                href={whatsappUrl}
                target="_blank"
            ...
            {/* CTA Card Mobile */}
            <div className="fixed z-10 border-t border-primary/20 bottom-0 left-0 right-0 w-full shadow px-4 py-2 bg-white md:hidden">
              <Link
                href={whatsappUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="bg-primary text-white w-full flex justify-center items-center gap-2 font-bold text-md md:text-md px-10 py-3 rounded-full shadow-xl transition-transform hover:scale-102 hover:cursor-pointer"
              >
                <MessageCircle className="mr-2 h-4 w-4" />
                {t.request_booking_whatsapp || 'Reservar no WhatsApp'}
              </Link>
